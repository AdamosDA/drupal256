...
menu:monitor:Mysql Administration panel: Mysql Administration panel  | Press ESC or Q to exit to main menu | (C)2012/drupal256.com
	 exec:Active Mysql Databases:disp:/usr/local/bin/menu/mysql-show_active_dbs.sh
	 exec:Active Mysql Users:disp:/usr/local/bin/menu/mysql-show_active_users.sh
	 ...
	 group:Change _Password for an Active Mysql User:
	 	...
	 	for I in `/usr/local/bin/menu/mysql-show_active_users.sh`; do \
	 	echo "exec:_$I:edit:/usr/local/bin/menu/mysql-change_user_password.sh $I ~Enter NEW password for user \"$I\":~"; \
	 	done
	 	show:::active_pusers
	 	remove:::active_pusers
	 	endgroup
	 nop
	 exec:Check and _optimize tables for active databases:disp:/usr/local/bin/menu/mysql-optimize_tables.sh
	 nop
	 exec:Current Mysql _Root Password:disp:cat ~drupal256/.my.cnf|tail -3 |head -2 
	 exec:Generate _New Mysql Root Password:disp:sudo /usr/local/bin/drupal256/drupal256-mysql-generate-passwd > /dev/null 2>&1;echo "New credentials generated.";echo "-------------------------- ";cat ~drupal256/.my.cnf|tail -3 |head -2
	 exit:E_xit
